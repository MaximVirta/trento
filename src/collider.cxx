...
NucleusPtr create_nucleus(const VarMap& var_map, std::size_t index) {
  ...
  const auto& nucleusConfigPath = var_map["nucleonConfigPath"].as<std::string>();
  // gaus1*lambda + gaus2*(1-lambda) -> gaus1(mean1, std1), gaus2(mean2, std2)

  double gamma = 0.0;

  if (gamma_std>0.0000001) {
    std::normal_distribution<double> ndist_gamma(gamma_mean, gamma_std); 
    gamma = ndist_gamma(random::engine);
  } else {
    gamma = gamma_mean;
  }
